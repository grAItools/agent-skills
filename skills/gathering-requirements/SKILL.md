...
description: Use when starting work on a new feature, product idea, defect report, or vague request — before any design or coding — to establish what is actually needed, for whom, and why, and to capture it as an agreed specification.
---
...
- A defect or pain report whose underlying need is unclear.
- A request that arrives as a solution ("add a button that...") rather than as a need.
- Before writing any design or plan for non-trivial work.

Do not use for mechanical, fully specified changes (rename X, bump a version) where the need is unambiguous.

## How to Work

**Dig, don't transcribe.** For every stated requirement, ask why it is needed and what the requester does today. Stated requests are often workarounds for a deeper need. Discover the underlying reason users do a thing, not just the way they currently do it, and record the *why* — it informs a thousand later implementation decisions.

**Work with a user to think like a user.** Direct contact with real users and domain experts beats profiles, tickets, and second-hand summaries. Watch how the work is actually done; the most valuable facts are the ones nobody thinks to mention.
...
Keep each requirement as the simplest abstract statement that accurately reflects the business need. Abstractions live longer than details.

**Build the shared vocabulary.** Agree on precise names for the domain concepts with the people who know the domain, and use those exact terms in every question, scenario, and document — and later in the code. Maintain a glossary. When a term feels awkward in conversation, the underlying concept is probably wrong: fix the concept, then the name. Projects fail when people call the same thing by different names, or different things by the same name.

**Write the user model down — better wrong than vague.** State explicitly who the users are, what they know, what they are trying to accomplish, in what environment, and how the user classes are weighted against each other. Where facts run out, guess concrete values and label them as guesses. A wrong explicit assumption gets questioned and corrected; a vague unspoken one silently steers every downstream decision. Without an explicit shared model, each contributor designs for a different imagined user and coherence is lost.
...
**Capture behavior as scenarios.** Write concrete usage scenarios: the actor, their goal, preconditions, the main success path, the important failure paths and variations, and expected frequency. Include boundary cases and the unglamorous realistic cases. Walk each scenario aloud with the domain experts — awkwardness you can hear is a misunderstanding you have not yet found. Scenarios double as acceptance criteria and, later, as tests. Even low-frequency scenarios can expose requirements nobody knew existed, so walk more of them than feels necessary.
...
**Track change.** Requirements will keep arriving after agreement. That is normal — but record each addition, who asked for it, and its cost in schedule and complexity, so "just one more feature" is a visible decision rather than silent drift.

## The Specification

Produce a single concise document (store it wherever the project keeps such documents; if there is no convention, ask or propose one):
...
Mark the document DRAFT until the stakeholders have confirmed it reflects their need; only then mark it AGREED and move on to design. Design and implementation should not begin from a draft understanding.
